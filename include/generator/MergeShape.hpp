...
namespace shape_generator
{
	...
	public:
		...
		private:
			...
			Edges(const MergeShape &shape) : head_{shape.head_.edges()},
		...
		private:
			...
			Vertices(const MergeShape &shape) : head_{shape.head_.vertices()},
		...
		MergeShape(Head head, Tail... tail) : head_{head},
											  tail_{tail...}
		{
		}
