...
namespace shape_generator
{
	...
	public:
		...
		private:
			...
			Triangles(const MergeMesh &mesh) : head_{mesh.head_.triangles()},
		...
		private:
			...
			Vertices(const MergeMesh &mesh) : head_{mesh.head_.vertices()},
		...
		MergeMesh(Head head, Tail... tail) : head_{std::move(head)},
											 tail_{std::move(tail)...}
		{
		}
