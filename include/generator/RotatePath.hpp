...
namespace shape_generator
{
	...
	public:
		RotatePath(Path path, const gml::dquat &rotation) : transformPath_{
																std::move(path),
		...
		RotatePath(Path path, double angle, const gml::dvec3 &axis) : RotatePath{std::move(path), gml::qrotate(angle, axis)}
		{
		}

		RotatePath(Path path, double angle, Axis axis) : RotatePath{
															 std::move(path),
															 gml::qrotate(
																 angle,
																 axis == Axis::X ? gml::dvec3{1.0, 0.0, 0.0} : (axis == Axis::Y ? gml::dvec3{0.0, 1.0, 0.0} : gml::dvec3{0.0, 0.0, 1.0}))}
		{
		}
