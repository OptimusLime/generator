...
namespace shape_generator
{
	...
	private:
		...
		explicit BezierShape(const ArrayWrapper &p, int segments) : mParametricShape{
																		[p](double t) {
																			ShapeVertex vertex;
	...
	public:
		...
		explicit BezierShape(const gml::dvec2 (&p)[D], int segments = 16) : // Work around a msvc lambda capture bug by wrapping the array.
