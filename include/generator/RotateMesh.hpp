...
namespace shape_generator
{
	...
	public:
		...
		RotateMesh(Mesh mesh, const gml::dquat &rotation) : transformMesh_{
																std::move(mesh),
		...
		RotateMesh(Mesh mesh, double angle, const gml::dvec3 &axis) : RotateMesh{std::move(mesh), gml::qrotate(angle, axis)}
		{
		}

		RotateMesh(Mesh mesh, double angle, Axis axis) : RotateMesh{
															 std::move(mesh),
															 gml::qrotate(
																 angle,
